...
paquetes <- c("httr","tidyquant","dplyr","lubridate","tidyr","ggplot2","patchwork","forcats","scales","viridis","plotly","gridExtra","ggplotify","grid","htmlwidgets")
...
# Definimos rutas para los datos crudos (raw) y los datos procesados (input) + creacion carpeta output
raw_dir <- "raw"
out_dir <- "input"
fin_dir <- "output"
if (!dir.exists(fin_dir)) {
  dir.create(fin_dir)
}
...
write.csv(df, file.path(fin_dir,"activos_pesos_RVS.csv"), row.names = FALSE)
...
ruta_out <- file.path(fin_dir, "grafico_8_5.png")
...
res <- httr::POST(
  url  = "https://0x0.st",
  body = list(file = httr::upload_file(ruta_out))
)

url_grafico <- httr::content(res, "text", encoding = "UTF-8") %>% trimws()
cat("Tu URL directo a la imagen es:\n", url_grafico, "\n")
...
ruta_out2 <- file.path(fin_dir, "grafico_9_5.png")
ggsave(
  filename = ruta_out2,
  plot     = sr_side,  # tu objeto patchwork + tabla
  width    = 10,        # en pulgadas (ajusta a tu gusto)
  height   = 6,         # en pulgadas
  dpi      = 300        # buena resolución
)

res <- POST(
  url  = "https://0x0.st",
  body = list(file = upload_file(ruta_out2))
)

# 3) Extrae el URL (0x0.st responde con un texto plano: la URL + "\n")
url_grafico <- content(res, "text", encoding = "UTF-8")
url_grafico <- trimws(url_grafico)   # quita saltos de línea sobrantes
...
ruta_out4 <- file.path(fin_dir, "Tabla_Var_Portofolios.png")
ggsave(
  filename = ruta_out4,
  plot     = tabla_gg,
  width    = 10,    # ancho en pulgadas
...
write.csv(plot_df, file.path(fin_dir,"Inversion_Activos_Inflacion.csv"), row.names = FALSE)


# 12.3. Gráfico de líneas: evolución de $100 en activos vs inflación
rentabilidad_vs_inf<- ggplot(plot_df, aes(x = year, y = index100, color = symbol)) +
...
ruta_out5 <- file.path(fin_dir, "rentabilidad_vs_inf.png")
ggsave(
  filename = ruta_out5,
  plot     = rentabilidad_vs_inf,
  width    = 10,    # ancho en pulgadas
  height   = 6,     # alto en pulgadas
  dpi      = 300    # resolución
)
...
write.csv(matriz_correlacion, file.path(fin_dir,"matriz_correlacion.csv"), row.names = FALSE)
...
mapa_correlaciones <- ggplot(matriz_correlacion_larga,
  ...
  geom_tile(color = "black", size = 0.4) +                                   # tu tile
  geom_text(aes(label = sprintf("%.2f", Correlation)),                       # etiqueta con 2 decimales
            color = "black", size = 3) +                                     # ajusta size según necesites
  scale_fill_gradientn(
    colours = c("#00008B", "#ffffff", "#ca0020"),
    values  = c(0, 0.3, 1),
    limits  = c(0, 1),
    name    = "Correlación"
  ) +
...
ruta_out6 <- file.path(fin_dir, "mapa_vs_betas_spy_side.png")
ggsave(
  filename = ruta_out6,
  plot     = mapa_vs_betas_spy_side,
  width    = 10,
  height   = 6,
  dpi      = 300,
  bg       = "transparent"      # aquí
